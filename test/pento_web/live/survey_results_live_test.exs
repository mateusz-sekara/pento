defmodule PentoWeb.SurveyResultsLiveTest do
  ...
  describe "Socket state" do
    ...
    test "no ratings exixt", %{socket: socket} do
      socket = assing_socket_details(socket)

      assert socket.assigns.products_with_average_ratings == [["Test Game", 0.0]]
    end
    ...
    defp assing_socket_details(socket) do
      socket =
        socket
        |> SurveyResultsLive.assign_age_group_filter()
        |> SurveyResultsLive.assign_gender_filter()
        |> SurveyResultsLive.assign_products()
      socket
    end
  end
end
